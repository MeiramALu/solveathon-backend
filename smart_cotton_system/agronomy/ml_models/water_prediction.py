...
class WaterAISuite:
    """
    Pipeline for Water Resource Management.
    ...
    def __init__(self, dry_threshold=30.0, target_humidity=35.0, max_daily_m3_mu=40.0, data_dir=None):
        ...
        # Set data directory - use Django BASE_DIR if available
        if data_dir is None:
            try:
                from django.conf import settings
                self.data_dir = os.path.join(settings.BASE_DIR, 'agronomy', 'data', 'dataset')
            except:
    ...
    def load_and_process_data(self):
        print(f" [1/5] Ingesting IoT & Weather Data from {self.data_dir}...")
        try:
            # Load Raw Data with full paths
            hum = pd.read_csv(os.path.join(self.data_dir, "CottonSensor", "HumiditySensor.csv"))
            tem = pd.read_csv(os.path.join(self.data_dir, "CottonSensor", "TemSensor.csv"))
            weather = pd.read_csv(os.path.join(self.data_dir, "Weather", "Weather.csv"))
            mgmt = pd.read_csv(os.path.join(self.data_dir, "ManagementInfo.csv"))
        ...
        # 7. Generate Target (Next Day's Humidity)
        ...
        # Save to agronomy/data directory
        output_path = os.path.join(os.path.dirname(self.data_dir), "daily_dataset.parquet")
    ...
    def train(self):
        if self.data is None:
            raise ValueError("No data loaded. Run load_and_process_data() first.")
    ...
    def add_irrigation_recommendation(
        self, df, target_humidity=35.0, max_daily_m3_mu=50.0
    ):
        """
        Adds two columns:
    ...
    def simulate_future(self, base_day_df: pd.DataFrame, days_ahead: int = 1):
        ...
        scenario_rain = df["rain(mm/day)"].iloc[0]
        scenario_temp = df["daily_mean_temperature(°C)"].iloc[0]
        ...
        # Set the future date
        last_date = pd.to_datetime(base_day_df["date"].iloc[0])
        ...
        future["Target_Tomorrow_Humidity"] = df[
            "sim_humidity"
        ]  # not really used, but keep shape
    ...
    def evaluate_and_plot(self):
        if self.model is None or self.preds is None:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.figures_dir, "Plot_1_Feature_Importance.png"))
        print(f"       Saved: {os.path.join(self.figures_dir, 'Plot_1_Feature_Importance.png')}")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.figures_dir, "Plot_2_Actual_vs_Pred.png"))
        print(f"       Saved: {os.path.join(self.figures_dir, 'Plot_2_Actual_vs_Pred.png')}")
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.figures_dir, "Plot_3_Residuals.png"))
        print(f"       Saved: {os.path.join(self.figures_dir, 'Plot_3_Residuals.png')}")

        # Digital Twin Simulation
        sample_loc = self.data[["loc_x", "loc_y"]].drop_duplicates().iloc[0]
        sensor_ts = self.data[
            (self.data["loc_x"] == sample_loc.loc_x)
            & (self.data["loc_y"] == sample_loc.loc_y)
        ].sort_values("date")

        sensor_X = sensor_ts[self.X_train.columns]
        sensor_preds = self.model.predict(sensor_X)

        plt.figure(figsize=(12, 6))
        plt.plot(
            sensor_ts["date"],
            sensor_ts["Target_Tomorrow_Humidity"],
            label="Actual Data",
            color="black",
            linewidth=2,
        )
        ...
        irr_events = sensor_ts[sensor_ts["irrigation_amount(m3/mu)"] > 0]
        if not irr_events.empty:
            plt.scatter(
                irr_events["date"],
                irr_events["Target_Tomorrow_Humidity"],
                color="blue",
                s=100,
        ...
        plt.title(
            f"Digital Twin: AI Tracking Soil Moisture for Sensor @ {sample_loc.loc_x}, {sample_loc.loc_y}"
        )
        plt.ylabel("Soil Humidity (%)")
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(self.figures_dir, "Plot_4_Digital_Twin.png"))
        print(f"       Saved: {os.path.join(self.figures_dir, 'Plot_4_Digital_Twin.png')}")
